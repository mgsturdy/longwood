...
export default function Gallery() {
  ...
  return (
    <section id="gallery" className="py-24 lg:py-32 bg-cream-v3" aria-labelledby="gallery-heading">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="inline-block rounded-full bg-sage/10 px-4 py-1.5 text-sm font-semibold text-sage mb-5">
            Take a Look
          </span>
          <h2 id="gallery-heading" className="text-3xl sm:text-4xl lg:text-5xl font-bold text-forest tracking-tight leading-tight">
            {GALLERY.title}
          </h2>
          <p className="mt-5 text-lg sm:text-xl text-warm-brown/80 leading-relaxed">{GALLERY.subtitle}</p>
        </div>

        <div className="flex flex-wrap justify-center gap-2 mb-12" role="tablist" aria-label="Gallery filter">
          {categories.map((cat) => (
            <button
              key={cat.value}
              role="tab"
        ...
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div className="rounded-2xl bg-soft-white border-2 border-dashed border-sage/30 p-10 text-center">
            <svg className="w-10 h-10 mx-auto text-sage/50 mb-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
